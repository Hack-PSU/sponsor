...
function ProjectsDirectory() {
  ...
  const filteredProjects = React.useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      const projectChallenges = project.categories.split(",").map((c) => c.trim())
      return selectedChallenges.some((selected) => projectChallenges.includes(selected))
    })
    ...
    <div className="bg-muted/30 text-foreground min-h-screen">
      <header className="bg-background/80 backdrop-blur-sm border-b sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          <h1 className="text-2xl font-bold tracking-tight">Project Directory</h1>
          <p className="text-sm text-muted-foreground">Browse projects and see which challenges they entered.</p>
        </div>
      </header>
